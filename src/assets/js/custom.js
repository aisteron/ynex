import {qs, qsa,toast} from './libs.js';
...
function listeners(){
	...
	qs('.navbar-toggler.filter').addEventListener("click", event => {

		qs("#filters").classList.contains('open')
		? close()
		: open()
	})
	...
	qs('button.close.btn').addEventListener("click", _ => close())

	// close by click anywhere

	document.addEventListener("click", event => {
		if(event.target == qs('button.close.btn')) return
		if(event.target == qs('.navbar-toggler.filter')) return
		if(qs('#filters .card-body.p-0').contains(event.target)) return
...
function open(){
  qs("#filters").classList.toggle("hidden")
...
function close(){
  qs("#filters").classList.remove("open")

	setTimeout(()=>{
		qs("#filters").classList.remove("hidden")
	},1000)
}

// true refresh

let sc = qs('script[type="module"]')
sc.setAttribute("src", sc.src+`?ts=${Date.now()}`)
...
// product page
	...
	function set_color(){
		if(!qs('.col-xl-12.colors')) return
		let colors = qsa('.col-xl-12.colors .list-group-item span')
		colors.forEach(c => {
	...
	function add_to_cart(){
		let button = qs('button.btn.btn-primary.label-btn')
		
		button.addEventListener("click", async event => {
			await toast.load()
			toast.show("Попап с текстом")
		})
	}
